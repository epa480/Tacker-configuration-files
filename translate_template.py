...
#/opt/stack/tacker/tacker/vnfm/infra_drivers/openstack/translate_template.py
    ...
    def _generate_hot_from_tosca(self, vnfd_dict, dev_attrs):
        ...
        try:
            translator = tosca_translator.TOSCATranslator(tosca,
                                                          parsed_params)
            name_yaml,heat_template_yaml, yaml_resources = translator.translate()
            LOG.debug("heat_template_yaml:%s", heat_template_yaml)
            LOG.debug("nested_resource_name:%s",nested_resource_name)
            if nested_resource_name:
                sub_heat_template_yaml = \
                    translator.translate_to_yaml_files_dict(
                        nested_resource_name)
                LOG.debug("sub_heat_template_yaml:%s", sub_heat_template_yaml)
                nested_resource_yaml = \
                    sub_heat_template_yaml[nested_resource_name]
                LOG.debug("nested resource name: %s", str(nested_resource_name))
                LOG.debug("nested resource yaml: %s", str(nested_resource_yaml))
        ...
        LOG.debug("self.nested_resources:%s", self.nested_resources)    

        if self.nested_resources:
            nested_tpl = toscautils.update_nested_scaling_resources(
                name_yaml,yaml_resources, mgmt_ports, metadata,
