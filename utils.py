...
#    under the License.
#/opt/stack/tacker/tacker/tosca/utils.py
...
def pre_process_alarm_resources(vnf, template, vdu_metadata):
    ...
    for policy in template.policies:
        if (policy.type_definition.is_derived_from(MONITORING)):
            query_metadata = \
                _process_query_metadata(vdu_metadata, policy)
...
def update_nested_scaling_resources(name_yaml,nested_resources, mgmt_ports, metadata,
    ...
    if nested_resources:
        ...
        if metadata:
            ...
            for vdu_name, metadata_dict in metadata['vdus'].items():
                LOG.debug(_('vdu_name %s |||| metadata_dict %s'), vdu_name, metadata_dict)
                LOG.debug(_('nested_resources_dict %s'), nested_resources_dict)
