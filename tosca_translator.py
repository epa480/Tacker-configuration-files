...
class TOSCATranslator(object):
    ...
    def translate(self):
        log.debug("Entrou novamente em translate")
        ...
        yaml_files = self.hot_template.output_to_yaml_files_dict(
            "output.yaml",
            self.node_translator.hot_template_version)
        log.debug('yaml_files:%s', str(yaml_files))
        for name, content in six.iteritems(yaml_files):
            log.debug('Name:%s', str(name))
            if name != "output.yaml":
                name_yaml = name
                log.debug('Traduziu e salvou')
                log.debug('name:%s', str(name))
                log.debug('content:%s', str(content))
                f = open('/usr/local/lib/python2.7/dist-packages/translator/hot/' + str(name), 'w+')
                # with open(name, 'w+') as f:
                f.write(content)
                f.close()
                yaml_resources = content
                
        # log.debug('yaml_files["output.yaml"]:%s',yaml_files["output.yaml"])        

        return name_yaml,yaml_files["output.yaml"],yaml_resources
